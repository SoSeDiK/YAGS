...
- Зажмите `XButton2` для прыжков!
  - Да, мне лень нажимать `Пробел`. Баннихоппинг тоже возможен.
...
- Нажмите `СКМ`, чтобы переключить режим автоматической ходьбы!
  - Нажмите `ПКМ` для переключения бега в режиме автоматической ходьбы.
  - `LShift` можно ипользовать для ручного управления бегом (полезно, чтобы войти в режим бега для баннихоппинга).
...
- Нажмите `СКМ` для:
  ...
  - Быстрой покупки выбранного предмета в чайнике (макс. количество).
...
- `Numpad 7` для перемотки времени к следующей ночи.
  - Зажмите `Numpad -` вначале для перемотки времени к следующему дню.
